...
from evidently.renderers.base_renderer import default_renderer, MetricRenderer
from evidently.renderers.html_widgets import header_text, plotly_figure
from plotly import graph_objs as go
from typing import List, Optional, Union

from model_training import preprocess_data, prepare_dataset, train_model, compute_isf_metrics, DATA_FILE, RANDOM_STATE, ISF_VARIABLES, TARGET
from predict import load_model, fit_model, compute_predictions
...
class MyMetric(Metric[MyMetricResult]):
    ...
    def calculate(self, data: InputData) -> MyMetricResult:
        ...
        pct_returning_cur, pct_inliers_cur = compute_isf_metrics(data.current_data, data.current_data[self.column_name])
        
        if data.reference_data is not None:
            pct_returning_ref, pct_inliers_ref = compute_isf_metrics(data.reference_data, data.reference_data[self.column_name])
        else:
...
class MyMetricRenderer(MetricRenderer):
    ...
    def render_html(self, obj: MyMetric) -> List[BaseWidgetInfo]:
        ...
        figure = go.Figure(data=[
            go.Bar(name='Current', x=['pct_returning', 'pct_inliers'], y=[metric_result.pct_returning_cur, metric_result.pct_inliers_cur], marker_color='rgb(255,0,0)'),  # Red for Current
            go.Bar(name='Reference', x=['pct_returning', 'pct_inliers'], y=[metric_result.pct_returning_ref, metric_result.pct_inliers_ref], marker_color='rgb(75,75,75)')  # Grey for Reference
        ])
        ...
        return [
            header_text(label=f"The percentage of returning customers within inliers is: {metric_result.pct_returning_cur}"),
            header_text(label=f"The percentage of inliers is: {metric_result.pct_inliers_cur}"),
...
def load_datasets():
    ...
    reference_val_data = pd.read_parquet(REFERENCE_DATA_PATH + 'reference_validation_data.parquet')
    ...
    #If the current dataset is more recent than the current training and validation data, I split it into training and validation set
    if os.stat(current_data_file).st_mtime > os.stat('../data/customer_churn_training_data.parquet').st_mtime:
        current_train_data, current_val_data = split_data(current_data, save_data=False)
        
    else:
        current_train_data = pd.read_parquet('../data/customer_churn_training_data.parquet')
        current_val_data = pd.read_parquet('../data/customer_churn_validation_data.parquet')
    
    return reference_data, reference_val_data, current_data, current_train_data, current_val_data   
...
def compute_predictions_df(pred_data: pd.DataFrame, isf, xgb_classifier): 
    ...
    X_pred = pred_data[pred_data['Predictions']==2].drop([TARGET,'Anomalies','Predictions'], axis=1)
...
@task
def generate_report(reference_data: pd.DataFrame, current_data: pd.DataFrame, report_name: str):
        
    print("Creating and running report...")
    
    if report_name not in ['drift', 'isolation_forest', 'xgboost']:
        print("Error: unknown report. report_name can only be \"drift\", \"isolation_forest\" or \"xgboost\".")
        return None
        
    elif (report_name == 'isolation_forest'):
...
def monitoring():
    ...
    reference_data, reference_val_data, current_data, current_train_data, current_val_data = load_datasets()
    ...
    reference_pred_data = compute_predictions_df(reference_val_data, isf, xgb_classifier)
    
    print("Predictions computed. Generating reports...")
    
    print("Generating Isolation Forest report...")
    isf_report = generate_report(reference_pred_data, current_pred_data, 'isolation_forest')
    ...
    pct_returning_diff = isf_report['metrics'][0]['result']['pct_returning_ref'] - isf_report['metrics'][0]['result']['pct_returning_cur']
    pct_inliers_diff = isf_report['metrics'][0]['result']['pct_inliers_ref'] - isf_report['metrics'][0]['result']['pct_inliers_cur']
    
    recall_diff = xgb_report['metrics'][0]['result']['reference']['recall'] - xgb_report['metrics'][0]['result']['current']['recall']
    f1_diff = xgb_report['metrics'][0]['result']['reference']['f1'] - xgb_report['metrics'][0]['result']['current']['f1']
    
    isf_drift = [drift_report['metrics'][0]['result']['drift_detected'], 
    ...
    if pct_returning_diff > 0.05 or pct_inliers_diff > 0.1:
        ...
        html_content = f"WARNING: Isolation Forest metrics lowered too much.\n The ratio of returning customers within anomalous data dropped by: {pct_returning_diff} and the ratio of inliers dropped by: {pct_inliers_diff} ."
    ...
    if True in isf_drift:
        print("Data drift detected in Isolation Forest variables. Sending email alert...")
        
        html_content = f"WARNING: A drift was detected in one or more of the employed to train the model Isolation Forest. You might want to investigate whether to change said variables. \n Drift detected in variables: {np.array(ISF_VARIABLES+[TARGET])[isf_drift].tolist()}"
    ...
    if data_drift == True:
        ...
        html_content = "WARNING:Data drift was detected. The models have been retrained."
        ...
        shutil.copy(CURRENT_DATA_PATH + 'customer_churn_training_data.parquet', REFERENCE_DATA_PATH + 'reference_training_data.parquet')
        shutil.copy(CURRENT_DATA_PATH + 'customer_churn_validation_data.parquet', REFERENCE_DATA_PATH + 'reference_validation_data.parquet')

...
TEST_DATAFRAME[CATEGORICAL_VARIABLES] = TEST_DATAFRAME[CATEGORICAL_VARIABLES].astype(
    'category'
)
...
def test_prepare_dataset():
    ...
    assert (
        actual_train.columns == expected_train.columns
    ).all(), "Training data columns mismatch"
    assert (
        actual_train.dtypes == expected_train.dtypes
    ).all(), "Training data types mismatch"
    assert (
        actual_train.values == expected_train.values
    ).all(), "Training data values mismatch"

    assert actual_val.shape == expected_val.shape, "Validation data shape mismatch"
    assert (
        actual_val.columns == expected_val.columns
    ).all(), "Validation data columns mismatch"
    assert (
        actual_val.dtypes == expected_val.dtypes
    ).all(), "Validation data types mismatch"
    assert (
        actual_val.values == expected_val.values
    ).all(), "Validation data values mismatch"
...
def test_compute_isf_metrics():
    ...
    actual_pct_returning, actual_pct_inliers = model_training.compute_isf_metrics(
        TEST_DATAFRAME, anomalies
    )
...
def test_preparing_xgboost_data():
    ...
    expected_train_matrix = xgb.DMatrix(
        expected_X_train, label=expected_y_train, enable_categorical=True
    )
    expected_val_matrix = xgb.DMatrix(
        expected_X_val, label=expected_y_val, enable_categorical=True
    )
    ...
    (
        ...
        actual_train_matrix,
        actual_val_matrix,
    ) = model_training.preparing_xgboost_data(TEST_TRAIN, TEST_VAL)

    assert actual_X_train.shape == expected_X_train.shape, "X_train Shape mismatch"
    assert (
        actual_X_train.columns == expected_X_train.columns
    ).all(), "X_train Columns mismatch"
    assert (
        actual_X_train.dtypes == expected_X_train.dtypes
    ).all(), "X_train Data types mismatch"
    assert (
        actual_X_train.values == expected_X_train.values
    ).all(), "X_train Values mismatch"

    assert actual_y_train.shape == expected_y_train.shape, "y_train Shape mismatch"
    assert (
        actual_y_train.dtypes == expected_y_train.dtypes
    ), "y_train Data types mismatch"
    assert (
        actual_y_train.values == expected_y_train.values
    ).all(), "y_train Values mismatch"

    assert actual_X_val.shape == expected_X_val.shape, "X_val Shape mismatch"
    assert (
        actual_X_val.columns == expected_X_val.columns
    ).all(), "X_val Columns mismatch"
    assert (
        actual_X_val.dtypes == expected_X_val.dtypes
    ).all(), "X_val Data types mismatch"
    ...
    assert (
        actual_train_matrix.get_data().shape == expected_train_matrix.get_data().shape
    ), "Dimensions of training DMatrices are not equal"
    assert (
        actual_train_matrix.get_data().toarray()
        == expected_train_matrix.get_data().toarray()
    ).all(), "Data of training DMatrices are not equal"
    assert all(
        actual_train_matrix.get_label()[i] == expected_train_matrix.get_label()[i]
        for i in range(len(expected_train_matrix.get_label()))
    ), "Labels of training DMatrices are not equal"

    assert (
        actual_val_matrix.get_data().shape == expected_val_matrix.get_data().shape
    ), "Dimensions of validation DMatrices are not equal"
    assert (
        actual_val_matrix.get_data().toarray()
        == expected_val_matrix.get_data().toarray()
    ).all(), "Data of validation DMatrices are not equal"
    assert all(
        actual_val_matrix.get_label()[i] == expected_val_matrix.get_label()[i]
        for i in range(len(expected_val_matrix.get_label()))
    ), "Labels of validation DMatrices are not equal"
...
def test_compute_predictions_df():
    ...
    xgb_classifier = XGB_CLASSIFIER.fit(
        TEST_TRAIN.drop('Exited', axis=1), TEST_TRAIN['Exited']
    )
    ...
    assert (
        actual_pred_data.columns == expected_pred_data.columns
    ).all(), "Columns mismatch"
    assert (
        actual_pred_data.dtypes == expected_pred_data.dtypes
    ).all(), "Types mismatch"
    assert (
        actual_pred_data.values == expected_pred_data.values
    ).all(), "Values mismatch"
